<?php
  ...
  /* This class must be included in another file and included later so we don't get an error about BloxBlockAPI class not existing. */

  class BloxArchitectBlock extends BloxBlockAPI {

    public $id = 'architect-block';
    public $name = 'Architect';
    public $options_class = 'BloxArchitectBlockOptions';
...
//		wp_enqueue_script('jquery-masonry');
    ...
    static function js_content( $block_id, $block, $original_block = NULL ) {
      ...
      $return_js = '';
      $settings  = BloxArchitectBlockOptions::get_settings( $block );
      ...
      foreach ( $pzarc_sections as $key => $pzarc_section ) {
        ...
        $pzarc_col_width = BloxBlocksData::get_block_width( $block['id'] ) / $pzarc_section['pzarc-cells-per-row-desktop'];
...
//                                    ));
    ...
    function content( $block ) {
      ...
      if ( function_exists( 'pzarc' ) ) {
        pzarc( $blueprint[0], $overrides, 'blox-block', NULL, $extra_overrides, $blueprint_tablet[0], $blueprint_phone[0] );
      }

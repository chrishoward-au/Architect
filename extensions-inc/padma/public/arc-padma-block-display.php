<?php
  ...
  /* This class must be included in another file and included later so we don't get an error about PadmaBlockAPI class not existing. */

  class PadmaArchitectBlock extends PadmaBlockAPI {

    public $id = 'architect-block';
    public $name = 'Architect';
    public $options_class = 'PadmaArchitectBlockOptions';
...
//		wp_enqueue_script('jquery-masonry');
    ...
    static function js_content( $block_id, $block, $original_block = NULL ) {
      ...
      $return_js = '';
      $settings  = PadmaArchitectBlockOptions::get_settings( $block );
      ...
      foreach ( $pzarc_sections as $key => $pzarc_section ) {
        ...
        $pzarc_col_width = PadmaBlocksData::get_block_width( $block['id'] ) / $pzarc_section['pzarc-cells-per-row-desktop'];
...
//                                    ));
    ...
    function content( $block ) {
      ...
      $extra_overrides['_blueprints_blueprint-title'] = ! empty( $settings['pzarc-blueprint-title'] ) ? $settings['pzarc-blueprint-title'] : NULL;


      $overrides['ids']              = $settings['pzarc-overrides-ids'];
      $overrides['tax']              = $settings['pzarc-overrides-taxonomy'];
      $overrides['terms']            = $settings['pzarc-overrides-terms'];
      $overrides['post__in']         = $settings['pzarc-overrides-ids'];
      $overrides['category__in']     = $settings['pzarc-cat_all_any'] === 'any' ? $settings['pzarc-category__in'] : NULL;
      $overrides['category__and']    = $settings['pzarc-cat_all_any'] === 'all' ? $settings['pzarc-category__in'] : NULL;
      $overrides['category__not_in'] = $settings['pzarc-category__not_in'];
      $overrides['tag__in']          = $settings['pzarc-tag__in'];
      $overrides['tag__not_in']      = $settings['pzarc-tag__not_in'];
      $overrides['tag__and']         = $settings['pzarc-tag_all_any'] === 'all' ? $settings['pzarc-tag__in'] : NULL;

      $overrides['panels_per_view'] = ( empty( $settings['pzarc-panels-per-view'] ) ? FALSE : $settings['pzarc-panels-per-view'] );
      ...
      if ( $settings['pzarc-custom-overrides'] ) {
        $custom_overrides = explode( "\n", str_replace( "\r", "", $settings['pzarc-custom-overrides'] ) );
        foreach ( $custom_overrides as $override ) {
          $exploded                  = explode( '=', $override );
          $overrides[ $exploded[0] ] = str_replace( '"', '', $exploded[1] );
        }
      }

      if ( function_exists( 'pzarc' ) ) {
        pzarc( $blueprint[0], $overrides, 'padma-block', NULL, $extra_overrides, $blueprint_tablet[0], $blueprint_phone[0] );
      }

<?php
  ...
  /* This class must be included in another file and included later so we don't get an error about PadmaBlockOptionsAPI class not existing. */

  class PadmaArchitectBlockOptions extends PadmaBlockOptionsAPI
  {
...
//         'custom' => 'If you set the Blueprint to custom, you can build you own here. You will still need pre-made Cells and Content Selections.',
    ...
    static function get_settings($block)
    {
      ...
      //
      // usage: $settings = PadmaExampleBlockOptions::get_settings($block)
      //  or    $settings = PadmaExampleBlockOptions::get_settings($block['id'])
      //
      ...
      foreach ($options as $option) {
        $settings[ $option[ 'name' ] ] = PadmaBlockAPI::get_setting($block, $option[ 'name' ], $option[ 'default' ]);
      }

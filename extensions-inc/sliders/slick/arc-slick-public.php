<?php
   ...
   * Time: 11:43 PM
   *
   * Loads the slick slider
   */

  add_filter('arc-set-slider-data', 'pzarc_slick_slider_data', 10, 2);
  function pzarc_slick_slider_data($slider, $blueprint)
  {
    $pzarc_slick_vers='1.8.0';
    // Slick
    wp_register_script('js-arc-front-slickjs', PZARC_PLUGIN_URL . '/extensions-inc/sliders/slick/arc-front-slickb.js', array('jquery'), PZARC_VERSION, true);
    wp_register_script('js-slickjs', PZARC_PLUGIN_URL . '/extensions-inc/sliders/slick/slick-'.$pzarc_slick_vers.'/slick/slick.min.js', array('jquery'), PZARC_VERSION, true);
    wp_register_style('css-slickjs', PZARC_PLUGIN_URL . '/extensions-inc/sliders/slick/slick-'.$pzarc_slick_vers.'/slick/slick.css',false,PZARC_VERSION);
    wp_register_style('css-arcslick', PZARC_PLUGIN_URL . '/extensions-inc/sliders/slick/arc-slick.css',false,PZARC_VERSION);
...
//      });';
  ...
  add_action('wp_print_footer_scripts', 'pzarc_slick_scripts');
  function pzarc_slick_scripts()
  {
    echo '<!-- Architect slider scripts -->';
    global $pzarchitect_slider_scripts;
    if (!empty($pzarchitect_slider_scripts)) {
      echo '<script type="text/javascript" id="architect-slick-'.rand(100000,999999).'">';
      echo '(function($){';

<?php
  ...
  function pzarc_celldef($def)
  {
    ...
    $celldefs[ 'post' ]['title'] = '<h1 class="entry-title">{{postlink}}{{title}}{{closepostlink}}</h1>';
    $celldefs[ 'post' ]['meta1'] = '<div class="entry-meta entry-meta1">{{meta1innards}}</div><!-- .entry-meta 1 -->';
    $celldefs[ 'post' ]['meta2'] = '<div class="entry-meta entry-meta2">{{meta2innards}}</div><!-- .entry-meta 2 -->';
    $celldefs[ 'post' ]['meta3'] = '<div class="entry-meta entry-meta3">{{meta3innards}}</div><!-- .entry-meta 3 -->';
    $celldefs[ 'post' ]['datetime'] = '<span class="date"><a href="{{permalink}}" title="{{title}}" rel="bookmark"><time class="entry-date" datetime="{{datetime}">{{fdatetime}}</time></a></span>';
    ...
    $celldefs[ 'post' ]['edit'] = '<span class="edit-link"><a class="post-edit-link" href="'.site_url().'/wp-admin/post.php?post={{postid}}&amp;action=edit">Edit</a></span>';
...
/*
    $celldefs[ 'page' ] = $celldefs[ 'post' ];
     ...
     $celldefs[ 'excerpt' ]['wrapper'] = '<div id="post-{{postid}}" class="pzarc-postexcerpt post-{{postid}} post type-{{posttype}} status-{{poststatus}} format-{{postformat}} hentry {{category-categories}} {{tag-tags}}">{{wrapperinnards}}</div>';
      ...
      $celldefs[ 'excerpt' ]['featuredimage'] = '<div class="entry-thumbnail"><img width="100%" src="{{imgsrc}}" class="attachment-post-thumbnail wp-post-image" alt="{{alttext}}">{{caption}}</div>';

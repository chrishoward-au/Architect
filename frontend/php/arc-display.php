...
//  $blueprint_info = new WP_Query('post_type=arc-blueprints&meta_key=_architect-blueprints_short-name&meta_value=' . $blueprint);
    ...
    $pzarc_blueprint[ 'section' ][ 1 ]
        ...
        'section-panel-settings' => (!empty($pzarc_blueprint[ 'blueprints_section-1-enable' ])
                ? pzarc_flatten_wpinfo(get_post_meta($pzarc_blueprint[ 'blueprints_section-1-panel-layout' ])) : null),
    );
    $pzarc_blueprint[ 'section' ][ 2 ]
        = array(
        'section-enable'         => !empty($pzarc_blueprint[ 'blueprints_section-2-enable' ]),
        'section-panel-settings' => (!empty($pzarc_blueprint[ 'blueprints_section-2-enable' ])
                ? pzarc_flatten_wpinfo(get_post_meta($pzarc_blueprint[ 'blueprints_section-2-panel-layout' ])) : null),
    );

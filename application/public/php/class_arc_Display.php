...
//  private $css = '/* Architect CSS */';
     ...
     *
     *************************************************/
    function __construct($blueprint_name, $blueprint_arr, $overrides, $is_shortcode)
    {
      $this->blueprint     = $blueprint_name;
      $this->blueprint_arr = $blueprint_arr;
    ...
    function render($pzarc_blueprint, $overrides, $is_shortcode = false)
    {
      ...
      // TODO: THIS HAS TO ALL WORK NICELY WITH CONTENT IN CONTENT. I.E. NO RECURSIVE ISSUES.

      $this->blueprint = $pzarc_blueprint;

      // Get the criteria
      if ($this->blueprint[ '_blueprints_content-source' ] == 'defaults' && !$is_shortcode)
      {
      ...
      else
      {
        ...
        $content_type = $this->blueprint[ '_blueprints_content-source' ];
      ...
      if (!empty($this->blueprint[ 'section' ]))
      {
        ...
        while (!empty($this->blueprint[ 'section' ][ $key ]))
        {
          ...
          $panelid = $this->blueprint[ '_blueprints_section-' . $key . '-panel-layout' ];
          ...
          if ($this->blueprint[ 'section' ][ $key ][ 'section-enable' ])
          {
            ...
            if ($this->blueprint[ '_blueprints_section-' . $key . '-layout-mode' ] == 'basic')
            {
            ...
            if ($pzarc_query->have_posts())
            {
              ...
              while ($pzarc_query->have_posts())
              {
                ...
                if ($this->blueprint[ '_blueprints_section-' . $key . '-panels-per-view' ] != 0 && ++$i > $this->blueprint[ '_blueprints_section-' . $key . '-panels-per-view' ])
                {
            ...
            $this->output .= ($key == 0) ? '{{nav-left-outside}}' : null;
            // Add the section nav
            //      var_dump($this->blueprint[ '_blueprints_navigation' ]);
            if ($this->blueprint[ '_blueprints_navigation' ] != 'none')
            {
              $this->output = str_replace(
                  '{{nav-' . $this->blueprint[ '_blueprints_navigator-position' ] . '-' . $this->blueprint[ '_blueprints_navigator-location' ] . '}}',
                  $this->add_nav(),
                  $this->output
              );
    ...
    {
      $this->output .= '<div id="pzarc-container-' . $this->blueprint[ '_blueprints_short-name' ] . '" class="pzarc-container swiper-container pzarc-blueprint-' . $blueprint_id . '">';
      if ($this->blueprint[ '_blueprints_pager' ] != 'none' && ($this->blueprint[ '_blueprints_pager-location' ] == 'top' || $this->blueprint[ '_blueprints_pager-location' ] == 'both'))
      {
        $this->output .= '{{pager}}';
      }
    }
    ...
    {
      if ($this->blueprint[ '_blueprints_pager' ] != 'none' && ($this->blueprint[ '_blueprints_pager-location' ] == 'bottom' || $this->blueprint[ '_blueprints_pager-location' ] == 'both'))
      {
        $this->output .= '{{pager}}';
      }
    ...
    {
      if ($this->blueprint[ '_blueprints_content-source' ] == 'default')
      {
        //don't change nuttin!
      ...
      else
      {
        ...
        switch ($the_criteria[ '_blueprints_content-source' ])
        {
...
// currently this is the only bit that really does anything
    ...
    function build_panel($post_info, $panel_def, $panelid, $section_panel_settings, $key)
    {
      ...
      // Open panel
      $this_panel = '<div class="pzarc-panel swiper-slide pzarc-{{classname}}" style="position:relative;width:' . $panel_width . '%;margin:' . ($this->blueprint[ '_blueprints_section-' . $key . '-panels-vert-margin' ] / 2) . '%;min-width:' . $panel_min_width . 'px;' . $panel_height . '">';
      ...
      $params = array('uid'         => $panelid . '_' . $post_info->ID,
                      ...
                      'type'        => $this->panel_info[ '_panels_design_image-resizing' ],
                      'backcolour'  => $this->panel_info[ '_panels_design_image-bgcolour' ],
                      'quality'     => $this->panel_info[ '_panels_design_image-quality' ],
      ...
      // }
      $layout = json_decode($this->panel_info[ '_panels_design_preview' ], true);
      // Get all the content data
      $the_title = get_the_title();
      switch ($this->panel_info[ '_panels_design_title-prefix' ])
      {
        case 'none':
          break;
        case 'bullet':
...
//      $the_inputs[ 'content' ]    = esc_html(preg_replace("/\\[pzarc(.)*\\]/uim", "zip", $the_content));
      ...
      $the_inputs[ 'date' ]       = apply_filters('the_date', get_the_date(empty($section_panel_settings[ '_panels_design_meta-date-format' ])
                                                                               ? 'l, F j, Y g:i a'
                                                                               : $section_panel_settings[ '_panels_design_meta-date-format' ]));
      ...
      if ($this->panel_info[ '_panels_design_thumb-position' ] == 'none' || empty($post_image))
      {
      ...
      {
        $panel_def[ 'image' ]   = str_replace('{{incontent}}', ' incontent pzarc-align ' . $this->panel_info[ '_panels_design_thumb-position' ], $panel_def[ 'image' ]);
      ...
      if (!empty($this->panel_info[ '_panels_design_link-titles' ]) && $this->panel_info[ '_panels_design_link-titles' ])
      {
      ...
      $panel_definition = (!empty($this->panel_info[ '_panels_design_image-captions' ]) && $this->panel_info[ '_panels_design_image-captions' ])
          ? str_replace('{{captioncode}}', $panel_def[ 'caption' ], $panel_definition) : $panel_definition;
      if ((!empty($this->panel_info[ '_panels_design_link-image' ]) && $this->panel_info[ '_panels_design_link-image' ] && $this->panel_info[ '_panels_design_link-image' ]) && $this->panel_info[ '_panels_design_background-position' ] == 'none')
      {
      ...
      if ($this->panel_info[ '_panels_design_background-position' ] == 'align' && !empty($the_inputs[ 'image' ]))
      {
        if ($this->panel_info[ '_panels_design_components-position' ] == 'bottom' || $this->panel_info[ '_panels_design_components-position' ] == 'right')
        {
          $this_panel = str_replace('{{bgimagetl}}', '<div class="pzarc-bg-image"><img class="nofill" src="' . $the_inputs[ 'image' ] . '" /></div>', $this_panel);
        }
        elseif ($this->panel_info[ '_panels_design_components-position' ] == 'top' || $this->panel_info[ '_panels_design_components-position' ] == 'left')
        {
      ...
      if ($this->panel_info[ '_panels_design_background-position' ] == 'fill' && !empty($the_inputs[ 'image' ]))
      {

        if ($this->panel_info[ '_panels_design_components-position' ] == 'bottom' || $this->panel_info[ '_panels_design_components-position' ] == 'right')
        {
          $this_panel = str_replace('{{bgimagetl}}', '<div class="pzarc-bg-image"><img class="fill ' . $this->panel_info[ '_panels_design_feature-scale' ] . '" src="' . $the_inputs[ 'image' ] . '" /></div>', $this_panel);
          $featureat  = ' abs-content';
        }
        elseif ($this->panel_info[ '_panels_design_components-position' ] == 'top' || $this->panel_info[ '_panels_design_components-position' ] == 'left')
        {
          $this_panel = str_replace('{{bgimagebr}}', '<div class="pzarc-bg-image"><img class="fill ' . $this->panel_info[ '_panels_design_feature-scale' ] . '" src="' . $the_inputs[ 'image' ] . '"/></div>', $this_panel);
          $featureat  = ' abs-content';
        }
      }
      ...
      $this->output = str_replace('{{width}}', $this->panel_info[ '_panels_design_image-max-width' ], $this->output);
    ...
    {
      $panel_meta1 = preg_replace('/%(\\w*)%/u', '{{$1}}', (!empty($section_panel_settings[ '_panels_design_meta1-config' ]) ? $section_panel_settings[ '_panels_design_meta1-config' ] : null));
      $panel_meta2 = preg_replace('/%(\\w*)%/u', '{{$1}}', (!empty($section_panel_settings[ '_panels_design_meta2-config' ]) ? $section_panel_settings[ '_panels_design_meta2-config' ] : null));
      $panel_meta3 = preg_replace('/%(\\w*)%/u', '{{$1}}', (!empty($section_panel_settings[ '_panels_design_meta3-config' ]) ? $section_panel_settings[ '_panels_design_meta3-config' ] : null));
    ...
    function add_pager()
    {
      ...
      $this->blueprint[ '_blueprints_pager' ]
          = ($this->blueprint[ '_blueprints_pager' ] == 'wppagenavi' && !function_exists('wp_pagenavi') ? 'prevnext'
          : $this->blueprint[ '_blueprints_pager' ]);

      switch ($this->blueprint[ '_blueprints_pager' ])
      {
        case 'names':
    ...
    {
      if (empty($this->panel_info) || empty($this->panel_info[ '_panels_design_excerpts-word-count' ]))
      {
        return $length;
      }

      return $this->panel_info[ '_panels_design_excerpts-word-count' ];
    }
    ...
    {
      if (empty($this->panel_info) || empty($this->panel_info[ '_panels_design_excerpts-linkmore' ]))
      {
        return $more;
      }

      if (!empty($this->panel_info[ '_panels_design_excerpts-linkmore' ]))
      {
        return esc_html($this->panel_info[ '_panels_design_excerpts-morestr' ]) . ' <a class="read-more" href="' . get_permalink(get_the_ID()) . '">' . esc_html($this->panel_info[ '_panels_design_excerpts-linkmore' ]) . '</a>';
      }
      else
      {
        return esc_html($this->panel_info[ '_panels_design_excerpts-morestr' ]);
      }
    }
  } //EOC

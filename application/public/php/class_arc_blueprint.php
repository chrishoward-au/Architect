<?php
	...
	class arc_Blueprint {
		...
		function __construct( $name, $device ) {
...
//          if (!empty($this->blueprint[ '_blueprints_section-' . ($i - 1) . '-panel-layout' ])) {
				$this->blueprint['section_object'][ $i ] = arc_SectionFactory::create( $i, $this->blueprint['section'][ ( $i - 1 ) ], $this->blueprint['_blueprints_content-source'], $this->blueprint['_blueprints_navigator'], $this->blueprint[ '_blueprints_section-' . ( $i - 1 ) . '-layout-mode' ], ( empty( $this->blueprint['_blueprints_slider-engine'] ) || $this->blueprint['_blueprints_slider-engine'] === 'slick15' ? 'slick' : $this->blueprint['_blueprints_slider-engine'] ), // Possible Future use
//                                       $this->blueprint[ '_blueprints_section-' . ($i - 1) . '-title' ],
					$titles, $this->blueprint['_blueprints_short-name'], $this->blueprint
...
// YET ONE OF THE BEAUTIIES OF SECTIONS IS WHEN YOU NEED TO CONTINUE. WITHOUT IT YOU'LL NEED TO USE SKIP POSTS
			...
			$this->blueprint['section'][0] = array(
				'section-enable'         => TRUE,

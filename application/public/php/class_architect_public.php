...
//      wp_reset_postdata(); // Pretty sure this goes here... Not after the query reassignment
      ...
      if ( $this->build->blueprint['_blueprints_content-source'] === 'defaults' && empty( $this->build->blueprint['_blueprints_section-0-panels-limited'] ) && ( class_exists( 'FLBuilderModel' ) && FLBuilderModel::is_builder_enabled() ) ) {
    ...
    private function render_this_architect_blueprint( $bp_nav_type, $bp_nav_pos, $bp_shortname, $caller, $bp_transtype, $panel_class, $content_class, $blueprint_type, $post_count ) {
      ...
      $arc_fs_location = NULL;
      if ( ! empty( $this->build->blueprint['_blueprints_masonry-features'] ) && ( in_array( 'filtering', $this->build->blueprint['_blueprints_masonry-features'] ) || in_array( 'sorting', $this->build->blueprint['_blueprints_masonry-features'] ) ) ) {
        $arc_fs_location = empty( $this->build->blueprint['_blueprints_masonry-filtering-controls-location'] ) ? 'top' : $this->build->blueprint['_blueprints_masonry-filtering-controls-location'];
      }
      ...
      if ( ! empty( $arc_fs_location ) ) {
//        if ( $arc_fs_location === 'right' || $arc_fs_location === 'bottom' ) {
//          echo '<div class="arc-filtering-and-sorting">';
//          do_action( "arc_masonry_controls_{$bp_shortname}" );
//          echo '</div>'; //close filtering and sorting
//        }
        echo '</div>'; // close masonry container. Only used when there's fs buttons
      }
      ...
      if ( ! empty( $this->build->blueprint['_blueprints_footer-text-link'] ) ) {
        ...
        echo do_shortcode( strip_tags( $this->build->blueprint['_blueprints_footer-text-link'], '<br><p><a><strong><em><ul><ol><li><pre><code><blockquote><h1><h2><h3><h4><h5><h6>' ) );
        echo '</div>';
      }
...
//      if (!$is_limited) {
      ...
      } elseif ( $is_builder_active && ( ! $is_limited && $this->build->blueprint['_blueprints_content-source'] === 'defaults' ) ) {
      ...
      // Sticky posts
      $this->criteria['ignore_sticky_posts'] = empty( $this->build->blueprint['_content_general_sticky'] );

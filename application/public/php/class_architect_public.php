...
// Don't use this in public as it reveals table names
    ...
    private function render_this_architect_blueprint( $bp_nav_type, $bp_nav_pos, $bp_shortname, $caller, $bp_transtype, $panel_class, $content_class, $blueprint_type ) {
      ...
      if ( ! empty( $this->build->blueprint[ '_blueprints_masonry-features' ] )
           && ( in_array( 'filtering', $this->build->blueprint[ '_blueprints_masonry-features' ] )
                || in_array( 'sorting', $this->build->blueprint[ '_blueprints_masonry-features' ] ) )
      ) {
      ...
      if ( ! empty( $arc_fs_location ) ) {
        ...
        $arc_fs_style=empty( $this->build->blueprint[ '_blueprints_masonry-filtering-controls-width' ] ) && !in_array($arc_fs_location,array('left','right'))?'':' style="flex-basis:'.$this->build->blueprint[ '_blueprints_masonry-filtering-controls-width' ].'%"';

<?php
  ...
  class arc_Section
  {
    ...
    {
      $data = $class::set_data($this->section[ 'section-panel-settings' ]);
...
//          $isotope = 'data-isotope-options=\'{ "layoutMode": "masonry","itemSelector": ".pzarc-panel","masonry":{"columnWidth":50,"gutter":20}}\'';
      ...
      if ($this->section[ 'section-panel-settings' ][ '_panels_design_background-position' ] != 'none' && ($this->section[ 'section-panel-settings' ][ '_panels_design_components-position' ] == 'bottom' || $this->section[ 'section-panel-settings' ][ '_panels_design_components-position' ] == 'right')) {
        $line_out = arc_Panel_post_bgimage::render('bgimage', $panel_def, $this->source, $data, $this->section[ 'section-panel-settings' ]);
...
//        var_dump(esc_html( $data['bgimage']));
      ...
      echo self::strip_unused_arctags(arc_Panel_post_Wrapper::render('components-open', $panel_def, '', $data, $this->section[ 'section-panel-settings' ]));
      ...
      foreach ($sequence as $component_type => $value) {
        if ($value[ 'show' ]) {
          ...
          $component_class    = $class.'_'.str_replace(array('1', '2', '3'), '', ucfirst($component_type));
...
//        echo '<div class="entry-content">',get_the_content(),'</div>';
      echo self::strip_unused_arctags(arc_Panel_post_Wrapper::render('components-close', $panel_def, '', $data, $this->section[ 'section-panel-settings' ]));

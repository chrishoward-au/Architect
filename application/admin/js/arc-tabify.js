...
jQuery(document).ready(function () {
  ...
  var slugdivs_panels = ['#redux-_architect-metabox-panels-settings', '#redux-_architect-metabox-panels-styling'];
...
//  var slugdivs_content = ['#default-filters', '#posts-filters', '#pages-filters', '#gallery-filters', '#slides-filters', '#custom-post-types-filters'];
  var title = 'h3 span';
  ...
  init();
  function init() {
      // some quirk in redux makes box disappear when u click a vertical tab
    return;
    ...
    if (jQuery('#panels').length) {
      ...
      for (x in slugdivs_panels) {
          if (x==0) {
          console.log(slugdivs_panels[x]);
              jQuery(slugdivs_panels[x]).addClass('tabs-pane').appendTo('#panels .tabs-content').show();
          } else {

...
//        $pzarc_mediaq_css .= '.pzarchitect .arc-slider-container.arc-slider-container-' . $pzarc_blueprints[ '_blueprints_short-name' ] . ' {'.$pzarc_panels[ '_panels_settings_panel-height-type' ].':' . $pzarc_panel_height . ';}' . $nl;
  ...
  function pzarc_breakpoint_css( $pzarc_mediaq_opener_css, &$pzarc_blueprints, $i, $pzarc_mediaq_css, $columns, $panel_width, $nl, $panels_class, $sections_class ) {
    ...
    if ( $pzarc_blueprints['_blueprints_section-0-panels-fixed-width'] ) {
      ...
      /// pzarc_truncate replaced round v1.10.1
      $rpercent     = pzarc_truncate( 100 / $columns, 2 ); // Since 1.6.1. IE11 fixes http://cruft.io/posts/percentage-calculations-in-ie/
      $rhmargin     = pzarc_truncate( (float) $hmargin, 2 ) . ( empty( $margin_units ) ? '%' : $margin_units ); // Since 1.6.1. IE11 fixes http://caniuse.com/#search=calc // 1.7 upped this to 2 decimals, Should still work in IE...
...
//            die();
        ...
        case ( $key == '_panels_design_preview' ):
          ...
          $left  = intval( $left );  // v1.17.0 Just in case it's blank
          $right = $pzarc_panels['_panels_design_image-spacing']['margin-right'];
          $right = preg_replace( "/([\\%px])/uiUm", "", $right );
          $right = intval( $right );  // v1.17.0 Just in case it's blank

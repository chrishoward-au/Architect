...
//      var_dump($wp_meta_boxes);
			...
			if ( is_admin() ) {
				...
				add_action( "redux/metaboxes/$this->redux_opt_name/boxes", array( $this, 'pzarc_mb_features_settings', ), 10, 1 );
				add_action( "redux/metaboxes/$this->redux_opt_name/boxes", array( $this, 'pzarc_mb_body_settings', ), 10, 1 );
				add_action( "redux/metaboxes/$this->redux_opt_name/boxes", array( $this, 'pzarc_mb_customfields_settings', ), 10, 1 );
				add_action( "redux/metaboxes/$this->redux_opt_name/boxes", array( $this, 'pzarc_mb_blueprint_content_selection', ), 10, 1 );
		...
		function pzarc_mb_customfields_settings( $metaboxes, $defaults_only = FALSE ) {
			...
			if ( is_admin() && ! empty( $_GET['post'] ) ) {
				...
				if ( $cfcount ) {
					...
					$pzarc_custom_fields =  apply_filters( 'arc_custom_field_list', $this->custom_fields, $this->source ) ;
					...
					for ( $i = 1; $i <= $cfcount; $i ++ ) {
						...
						$sections[] = array(
							...
							'fields'     => array(
								...
								array(
									'title'    => __( 'Content source', 'pzarchitect' ),
									'id'       => $prefix . 'cfield-' . $i . '-content-source',
									'type'     => 'select',
									'default'  => '',
									'options'  => array(
									  ''=> 'Select source',
                    'custom_field'=>__('Custom field','pzarchitect'),
                    ...
                    'tablefield'=>__('Any field from any database table','pzarchitect')
									...
									'subtitle' => __( 'If a custom field is not shown in the dropdown, 
									it is either because it has no data yet or 
									the custom field list cache needs clearing. Go to Architect > Tools and clear the caches.
									Or you need to enable the hhiden custom fields;
									or the field is in a custom table', 'pzarchitect' ),

								),
								array(
									'title'    => __( 'Custom field name', 'pzarchitect' ),
                ...
                array(
                  'title'    => __( 'Table name', 'pzarchitect' ),
                  'id'       => $prefix . 'cfield-' . $i . '-table-name',
                  'type'     => 'select',
                  'default'  => '',
                  'options'  => $pzarc_custom_fields,
                ),
                array(
                  'title'    => __( 'Table field name', 'pzarchitect' ),
                  'id'       => $prefix . 'cfield-' . $i . '-table-field-name',
                  'type'     => 'select',
                  'default'  => '',
                  'options'  => $pzarc_custom_fields,
                ),

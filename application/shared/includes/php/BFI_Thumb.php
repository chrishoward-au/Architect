  ...
  if (!function_exists('bfi_wp_image_editor_check_notice')) {
    function bfi_wp_image_editor_check_notice()
    {
      printf("<div class='error'><p>%s</div>",
             __("The server does not have ImageMagick or GD installed and/or enabled! Any of these includes are required for WordPress to be able to resize images. Please contact your server administrator to enable this before continuing.", "default"));
    }
  }
...
// Crop is always applied (just like timthumb)
  ...
  if (!function_exists('bfi_image_resize_dimensions')) {
    ...
    {
      $aspect_ratio = ($crop ? 1 : $orig_w / $orig_h);
//var_dump($payload, $orig_w, $orig_h, $dest_w, $dest_h, $crop);
 ...
 //     var_dump(array(0, 0, (int)$s_x, (int)$s_y, (int)$new_w, (int)$new_h, (int)$crop_w, (int)$crop_h));

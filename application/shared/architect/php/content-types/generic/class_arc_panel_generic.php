...
//      }
  ...
  public function get_title(&$post)
  {
    ...
    if ('thumb' === $this->section['_panels_design_title-prefix']) {
      ...
      $focal_point = (empty($focal_point) ? array(50,
...
//        $this->data[ 'meta' ][ 'fdatetime' ] = date_i18n(strip_tags($this->section[ '_panels_design_meta-date-format' ]), str_replace(',', ' ', strtotime(get_the_date())));
  ...
  public function get_image(&$post)
  {
    ...
    //Use lorempixel
    if (empty($this->data['image']['image']) && !empty($this->section['_panels_design_use-filler-image-source']) && 'none' !== $this->section['_panels_design_use-filler-image-source'] && 'specific' !== $this->section['_panels_design_use-filler-image-source']) {
...
//        $imageURL = 'http://lorempixel.com/' . $image_grey . $width . '/' . $height . '/' . $post[ 'image' ][ 'original' ];
      ...
      $this->data['image']['original'] = !$is_offline ? array($imageURL,
                                                              $width,
                                                              $height,
                                                              false) : false;
    ...
    if (empty($this->data['image']['image']) && 'specific' === $this->section['_panels_design_use-filler-image-source'] && !empty($this->section['_panels_design_use-filler-image-source-specific']['url'])) {
      ...
      $this->data['image']['original'] = array($imageURL,
                                               $width,
                                               $height,
                                               false);
      $this->data['image']['caption']  = '';

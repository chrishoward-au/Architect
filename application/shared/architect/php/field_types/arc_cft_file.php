   ...
   */
    ...
    function arc_cft_file_get(&$i,&$section,&$post,&$postmeta,$data){
      ...
      if ( ! empty( $data['data']['value'] ) ) {
        $content = '<a href="' . $data['data']['value'] . '" class="pzarc-acf-file" target="'.$data['data']['link-behaviour'].'">' . ($data['data']['link-text']!='<span class="pzarc-link-text"></span>'?$data['data']['link-text']:'<span class="pzarc-link-text">'.$data['data']['value'].'</span>') . '</a>';
      }

      ...
      public function _enqueue() {
        ...
        if ( $pagenow == "post-new.php" || $pagenow == "post.php" ) {
          ...
          if ( in_array( $post->post_type, $types ) ) {
            ...
            wp_enqueue_style( 'redux-extension-metaboxes-css', apply_filters( "redux/metaboxes/{$this->parent->args['opt_name']}/enqueue/redux-extension-metaboxes-css", $this->_extension_url . 'extension_metaboxes.css' ), '', filemtime( $this->_extension_dir . 'extension_metaboxes.css' ), // todo - version should be based on above post-filter src
                'all' );
            ...
            wp_enqueue_script( 'redux-extension-metaboxes-js', apply_filters( "redux/metaboxes/{$this->parent->args['opt_name']}/enqueue/redux-extension-metaboxes-js", $this->_extension_url . 'extension_metaboxes' . Redux_Functions::isMin() . '.js' ), array(
                    'jquery',
                    'redux-js',
                ), self::$version, 'all' );
          ...
          if ( isset( $box['post_types'] ) && ! empty( $box['post_types'] ) ) {
            foreach ( $box['post_types'] as $posttype ) {
              ...
              if ( isset( $box['args'] ) ) {
                ...
                if ( $this->parent->args != $this->orig_args ) {
            ...
            <?php
              ...
              foreach ( $sections as $sKey => $section ) :
                ...
                if ( isset( $section['fields'] ) && ! empty( $section['fields'] ) ) {
                  ...
                  if ( isset( $section['fields'][0][0]['id'] ) ) {
                    // v1.15.0 Fix PHP notice
                    echo '<tr><td>'.__('Not selected for animation','pzarchitect').'</td></tr>';
                  } else {

    ...
    function render() {
    	...
    	if ( isset( $this->field['options'] ) && !empty( $this->field['options'] ) ) {
			...
			foreach($this->field['options'] as $k => $v){
				...
				echo '<input type="text" id="' . $this->field['id'] . '-text-' . $k . '" name="' . $this->field['name'] . '['.$k.']' . $this->field['name_suffix'] . '" ' . $placeholder . 'value="' . esc_attr($this->value[$k]) . '" class="regular-text ' . $this->field['class'] . '" '.(!empty($this->field['readonly'])?'readonly':null).'/><br />';
				
			}//foreach
    		
    	} else {
    		
    		echo '<input type="text" id="' . $this->field['id'] . '-text" name="' . $this->field['name'] . $this->field['name_suffix'] . '" ' . $placeholder . 'value="' . esc_attr($this->value) . '" class="regular-text ' . $this->field['class'] . '" '.(!empty($this->field['readonly'])?'readonly':null).'/>';
    	}

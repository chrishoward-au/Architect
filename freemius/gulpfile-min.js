...
gulp.task('prepare-source', function () {
    gulp.src('**/*.php').pipe(sort()).pipe(wpPot({
        ...
        gettextFunctions: [{ name: 'get_text_inline' }, { name: 'fs_text_inline' }, { name: 'fs_echo_inline' }, { name: 'fs_esc_js_inline' }, { name: 'fs_esc_attr_inline' }, { name: 'fs_esc_attr_echo_inline' }, { name: 'fs_esc_html_inline' }, { name: 'fs_esc_html_echo_inline' }, { name: 'get_text_x_inline', context: 2 }, { name: 'fs_text_x_inline', context: 2 }, { name: 'fs_echo_x_inline', context: 2 }, { name: 'fs_esc_attr_x_inline', context: 2 }, { name: 'fs_esc_js_x_inline', context: 2 }, { name: 'fs_esc_js_echo_x_inline', context: 2 }, { name: 'fs_esc_html_x_inline', context: 2 }, { name: 'fs_esc_html_echo_x_inline', context: 2
            /*,
              {name: '_fs_text'},
    ...
    return gulp.src(languagesFolder + 'freemius.pot').pipe(pofill({
        items: function items(item) {
    ...
    })).pipe(rename('freemius-en.po')).pipe(gulp.dest(languagesFolder));
});

// Push updated po resource to transifex.
gulp.task('update-transifex', ['prepare-source'], function () {
    return gulp.src(languagesFolder + 'freemius-en.po').pipe(transifex.pushResource());
});
...
gulp.task('prepare-translations', ['download-translations'], function () {
    ...
    return folders.map(function (folder) {
        return gulp.src(path.join(languagesFolder, folder, 'freemius-en.po')).pipe(rename('freemius-' + folder + '.po')).pipe(gulp.dest(languagesFolder));
    });
});

// Feel up empty translations with English.
gulp.task('translations-feelup', ['prepare-translations'], function () {
    return gulp.src(languagesFolder + '*.po').pipe(pofill({
        items: function items(item) {
            // If msgstr is empty, use identity translation
...
gulp.task('cleanup', ['prepare-translations'], function () {
    ...
    return folders.map(function (folder) {
        return gulp.src(path.join(languagesFolder, folder), { read: false }).pipe(clean());
    });
});
...
gulp.task('compile-translations', ['translations-feelup'], function () {
    // Compile POs to MOs.
    return gulp.src(languagesFolder + '*.po').pipe(gettext()).pipe(gulp.dest(languagesFolder));
});
